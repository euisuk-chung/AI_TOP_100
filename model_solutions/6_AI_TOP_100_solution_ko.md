...
### Q1. 30명의 입국 신청자 심사 결과

**접근법**: 25개의 상세 규정을 파악하고, 7가지 입국 서류(여권, 비자, 입국 신고서 등)의 데이터를 규칙에 따라 자동으로 검증하는 시스템을 구현합니다.

**가이드**:

1. **규칙 파일 분석**: `inspection_rules.txt`에서 25개 규칙을 추출하고 우선순위 결정
2. **데이터 로드**: 30명의 신청자 서류 데이터를 JSON으로 파싱
3. **규칙 적용**: 각 신청자에 대해 규칙 1번부터 순차적으로 검증
4. **결과 생성**: 위반 시 첫 번째(가장 낮은 번호) 위반 규칙을 사유로 거부

**핵심 규칙 예시** (실제 규칙은 `inspection_rules.txt` 참조):
- 규칙 1: 여권 유효기간이 심사일 기준 6개월 이상 남아있어야 함
- 규칙 2: 비자 유형이 방문 목적과 일치해야 함
- 규칙 3: 입국 신고서의 모든 필수 항목이 작성되어야 함
- 규칙 4: 체류 예정 기간이 비자 허용 기간 이내여야 함
- ...

**검증 체크리스트**:
- [ ] 여권 유효기간 확인 (심사일 2025-11-22 기준)
- [ ] 비자 유형과 목적 일치 확인
- [ ] 필수 서류 누락 여부
- [ ] 금액/기간 등 수치 기준 충족
- [ ] 복수 위반 시 최소 번호 사유 반환

**정답 형식**: 30명 전원에 대한 심사 결과 JSON 배열
...
**정답**: 제공된 `applicants.json`과 `inspection_rules.txt`를 분석하여 25개 규칙을 정확히 적용한 결과를 제출합니다. 실제 정답은 데이터 파일의 구체적 내용에 따라 결정됩니다.
