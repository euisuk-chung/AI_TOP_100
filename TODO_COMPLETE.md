...
**최초 작성일**: 2025-12-19
**최종 갱신**: 2026-05-15 — 시즌 3/4/5 13개 챌린지 추가, 파일 스키마 `S{n}_{round}_{idx}_{title}.md` 적용, source 폴더를 code 기반으로 재명명
**총 문제 수**: 22개
...
| 문제 | 상태 | 세부 진행률 | 필요 작업 |
|------|------|-------------|-----------|
| S1 예선 1 춘식도락 메뉴 분석 (menu) | ✅ 완료 | 5/5 | - |
| S1 예선 2 고대 유적의 비밀 (crypt) | ✅ 완료 | 3/3 | - |
| S1 예선 3 영상 팩트 체크 (ageofai) | ⏳ 부분 | 3/7 | VLM 영상 분석 |
| S1 예선 4 전투 게임 시뮬레이션 (modeling) | ✅ 완료 | 6/6 | - |
| S1 예선 5 PDF 속 텍스트 추적 (textfinder) | ⏳ 부분 | 3/4 | 5번째 노래 찾기 |
| S1 본선 1 AI 입국 심사관 (immigration) | ❌ 미해결 | 0/1 | 이미지 PDF OCR |
| S1 본선 2 몽타주를 그려라 (montage) | ❌ 미해결 | 0/1 | API 제출 |

### 시즌 2 (#2, 2025-12)

| 문제 | 상태 | 세부 진행률 | 필요 작업 |
|------|------|-------------|-----------|
| S2 본선 1 인수인계 (handover) | ⏳ 부분 | 2/4 | 음성 Transcript |
| S2 본선 2 스파이의 요리코드 (cooking) | ❌ 미해결 | 0/5 | 인터프리터 구현 |

### 시즌 3 (#3, 2026-01) ← 신규

| 문제 | 상태 | 세부 진행률 | 필요 작업 |
|------|------|-------------|-----------|
| S3 본선 1 뉴스 속 혁신가를 발굴하라 (news) | ❌ 미시작 | 0/5 | 자료 다운로드, 588건 URL 분석 |
| S3 본선 2 고대 유적의 비밀 II 아스키 미로 (ascii) | ❌ 미시작 | 0/3 | 자료 다운로드, OCR + 코드 실행 |
| S3 본선 3 주차의 달인 (parking) | ❌ 미시작 | 0/4 | 자료 다운로드, 시뮬레이터 활용 |

### 시즌 4 (CAMPUS 예선, 2026-04) ← 신규

| 문제 | 상태 | 세부 진행률 | 필요 작업 |
|------|------|-------------|-----------|
| S4 예선 1 2026 수능 그날의 대화 (csat) | ❌ 미시작 | 0/4 | 자료 다운로드 |
| S4 예선 2 디지털 아트 감정사 (art-detective) | ❌ 미시작 | 0/4 | 자료 다운로드 |
| S4 예선 3 프리라이더를 찾아라 (freerider) | ❌ 미시작 | 0/6 | 자료 다운로드 |
| S4 예선 4 웹사이트 버그 찾기 (webqa) | ❌ 미시작 | 0/6 | 자료 다운로드 |
| S4 예선 5 1906 오늘 한성은 (newspaper) | ❌ 미시작 | 0/5 | 자료 다운로드 |

### 시즌 5 (CAMPUS 본선, 2026-04) ← 신규

| 문제 | 상태 | 세부 진행률 | 필요 작업 |
|------|------|-------------|-----------|
| S5 본선 1 수능 경향 분석 (csatmyth) | ❌ 미시작 | 0/4 | 자료 다운로드 |
| S5 본선 2 AI 수사관 잠복 근무 (stakeout) | ❌ 미시작 | 0/4 | 자료 다운로드 |
| S5 본선 3 스타트업 창업 여정 (startup) | ❌ 미시작 | 0/4 | 자료 다운로드 |
| S5 본선 4 웹소설 플랫폼 복구 (webnovel) | ❌ 미시작 | 0/6 | 자료 다운로드 |
| S5 본선 5 시간 여행자가 나타났어요 (newspaper-final) | ❌ 미시작 | 0/6 | 자료 다운로드 |

> 시즌 3/4/5는 본 갱신 시점에 신규 추가되었으며, `source/{code}/` 폴더는 README.md만 들어있고 실제 챌린지 자료(zip 등)는 아직 받지 않았습니다. challenge.aitop100.org 로그인 후 각 챌린지의 자료를 받아 해당 폴더에 압축 해제하세요.
...
### Transcript 파일 위치
```
d:\repo\AI_TOP_100\source\ageofai\transcript\
├── 적정선은 어디인가.txt
├── AI를 통한 치유.txt
...
**Source**: source/immigration (applicants/, inspection_rules.txt)
...
# OCR 실행 (이미지는 이미 변환됨)
cd d:/repo/AI_TOP_100/source/immigration
...
**Source**: source/handover (메일, 캘린더, 메모, 책상, 통화)
...
### 음성 파일 정보
```
d:\repo\AI_TOP_100\source\handover\5_통화\5_통화\
├── 2025_04_30.m4a
├── 2025_05_20.m4a
...
**Source**: source/cooking (1.txt ~ 5.txt, cookingguide.png)
